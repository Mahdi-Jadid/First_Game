...
public class Window {
   ...
   public static void init (Window_Configuration... window_settings)  {
        if (window == null) {
    ...
    public static void init (int width, int height, Window_Configuration... window_settings) {
        if (window == null) {
            CUSTOM_WIDTH = width;
            CUSTOM_HEIGHT = height;
            window = new Window(CUSTOM_WIDTH, CUSTOM_HEIGHT, TITLE);
            for (var setting : window_settings)
                setting.configure();
        }
    }

    public static void init (int width, int height, String title, Window_Configuration... window_settings) {
        if (window == null) {
            CUSTOM_WIDTH = width;
            CUSTOM_HEIGHT = height;
            CUSTOM_TITLE = title;
            window = new Window(CUSTOM_WIDTH, CUSTOM_HEIGHT, CUSTOM_TITLE);
            for (var setting : window_settings)
                setting.configure();
        }
    }

    public static void init (String title, Window_Configuration... window_settings) {
        if (window == null) {
            CUSTOM_TITLE = title;
            window = new Window(WIDTH, HEIGHT, CUSTOM_TITLE);
            for (var setting : window_settings)
                setting.configure();
        }
    }
